    ...
    <>
      <div
        className={cn(
          ...
          "bg-card border border-border rounded-xl shadow-[0_8px_30px_-4px_hsl(var(--foreground)/0.12)]",
          "px-4 py-2.5 flex items-center gap-3",
          "animate-[slide-up_0.3s_cubic-bezier(0.34,1.56,0.64,1)]"
        )}
      >
        <div className="flex items-center gap-2 pr-3 border-r border-border">
          <span className="text-xs font-semibold text-foreground">{selectedCount} selected</span>
          <button onClick={onClear} className="w-5 h-5 rounded-full bg-muted hover:bg-destructive/10 hover:text-destructive flex items-center justify-center transition-colors">
            <X className="w-3 h-3" />
          </button>
        </div>
